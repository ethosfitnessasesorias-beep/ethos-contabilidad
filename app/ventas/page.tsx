    ...
    <div>
      ...
      <div className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/40">
        ...
        {cargando ? (
          ...
          (() => {
            // Agrupación año → trimestre → mes con subtotales (estilo Holded)
            const claves = (f: (typeof visibles)[number]) => {
              const a = f.fecha_emision.slice(0, 4);
              const t = `${a}·T${Math.floor((Number(f.fecha_emision.slice(5, 7)) - 1) / 3) + 1}`;
              return { a, t, m: f.fecha_emision.slice(0, 7) };
            };
            const totalA = new Map<string, number>();
            const totalT = new Map<string, number>();
            const totalM = new Map<string, number>();
            for (const f of visibles) {
              const { a, t, m } = claves(f);
              totalA.set(a, (totalA.get(a) ?? 0) + Number(f.total));
              totalT.set(t, (totalT.get(t) ?? 0) + Number(f.total));
              totalM.set(m, (totalM.get(m) ?? 0) + Number(f.total));
            }
            const nombreMes = (m: string) =>
              new Date(m + "-01T00:00:00").toLocaleDateString("es-ES", { month: "long" });
            ...
            for (const f of visibles) {
              ...
              if (a !== pa) {
                nodos.push(
                  <div key={`a-${a}`} className="flex items-center justify-between border-b border-zinc-800 bg-zinc-900 px-4 py-2">
                    <span className="text-sm font-black text-white">{a}</span>
                    <span className="text-sm font-black tabular-nums text-white">{eur(totalA.get(a) ?? 0)}</span>
                  </div>
                );
                pa = a; pt = ""; pm = "";
              }
              if (t !== pt) {
                nodos.push(
                  <div key={`t-${t}`} className="flex items-center justify-between border-b border-zinc-800 bg-zinc-900/70 px-4 py-1.5">
                    <span className="text-xs font-bold uppercase tracking-wider text-zinc-400">{t.split("·")[1]}</span>
                    <span className="text-xs font-bold tabular-nums text-zinc-300">{eur(totalT.get(t) ?? 0)}</span>
                  </div>
                );
                pt = t; pm = "";
              }
              if (m !== pm) {
                nodos.push(
                  <div key={`m-${m}`} className="flex items-center justify-between border-b border-zinc-800 bg-zinc-900/40 px-4 py-1">
                    <span className="text-[11px] font-bold capitalize text-zinc-500">{nombreMes(m)}</span>
                    <span className="text-[11px] font-bold tabular-nums text-zinc-400">{eur(totalM.get(m) ?? 0)}</span>
                  </div>
                );
                pm = m;
              }
              nodos.push(filaFactura(f));
            }
            return nodos;

            // Fila de factura (declaración: se eleva dentro del IIFE)
            function filaFactura(f: Factura) {
              const saldo = saldos.get(f.id);
