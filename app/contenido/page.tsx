...
export default function ContenidoPage() {
  ...
  function abrirNuevo() {
    setEd(null);
    setF({ titulo: "", fase: "idea", plataforma: "instagram", formato: "reel", responsable: personas[0]?.codigo });
    setModal(true);
  }
          ...
          <div className="flex items-center gap-2">
            <div className="flex rounded-xl bg-zinc-900 p-1">
              {(["tablero", "calendario"] as const).map((v) => (
                <button key={v} onClick={() => setVista(v)} className={`rounded-lg px-3 py-1.5 text-sm font-bold capitalize ${vista === v ? "bg-red-600 text-white" : "text-zinc-400"}`}>{v}</button>
              ))}
            </div>
        ...
        {vista === "tablero" ? (
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-4">
            <div className="mb-3 flex items-center justify-between">
              <button onClick={() => setMesCal(new Date(mesCal.getFullYear(), mesCal.getMonth() - 1, 1))} className="rounded-lg bg-zinc-800 px-3 py-1.5 text-sm font-bold text-zinc-300">←</button>
              <span className="text-sm font-black capitalize text-white">{mesCal.toLocaleDateString("es-ES", { month: "long", year: "numeric" })}</span>
              <button onClick={() => setMesCal(new Date(mesCal.getFullYear(), mesCal.getMonth() + 1, 1))} className="rounded-lg bg-zinc-800 px-3 py-1.5 text-sm font-bold text-zinc-300">→</button>
            </div>
            <div className="grid grid-cols-7 gap-1.5">
              {["L", "M", "X", "J", "V", "S", "D"].map((d) => (
                <div key={d} className="text-center text-[11px] font-bold text-zinc-600">{d}</div>
              ))}
              {celdas.map((dia, i) => (
                <div key={i} className={`min-h-20 rounded-lg p-1.5 ${dia ? "bg-zinc-950" : ""}`}>
                  {dia && (
                    <>
                      <p className="mb-1 text-[11px] font-bold text-zinc-500">{dia}</p>
                      <div className="flex flex-col gap-1">
                        {postsDelDia(dia).map((c) => {
                          const fmt = formatoDe(c.formato);
                          return (
                            <button key={c.id} onClick={() => abrirEditar(c)} className={`${fmt.clase} truncate rounded px-1.5 py-1 text-left text-[10px] font-bold text-white`}>
