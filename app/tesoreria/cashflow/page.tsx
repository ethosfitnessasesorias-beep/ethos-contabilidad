...
export default function Tesoreria() {
  ...
  // Escenarios: ajuste % sobre ingresos/gastos recurrentes (se guardan en config_texto)
  ...
  // Proyección de saldo mes a mes, con escenarios (ajuste % sobre ingresos y gastos)
  const proyeccion = useMemo(() => {
    const filas: { mes: string; ingresos: number; gastos: number; saldos: number[] }[] = [];
    const activos = recurrentes.filter((r) => r.activo);
    const saldosAcum = escenarios.map(() => saldoActual);
    const hoy = new Date();
    for (let i = 1; i <= meses; i++) {
      ...
      escenarios.forEach((e, k) => {
        saldosAcum[k] += ingresos * (1 + e.ing / 100) - gastos * (1 + e.gas / 100);
      });
      filas.push({ mes: mesNombre(primerDia), ingresos, gastos, saldos: [...saldosAcum] });
    }
    return filas;
  }, [recurrentes, saldoActual, meses, escenarios]);
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-5">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => setEditEscenarios(!editEscenarios)}
          ...
          <div className="mb-3 flex flex-col gap-2 rounded-xl border border-zinc-800 bg-zinc-950/60 p-3">
            {escenarios.map((e, k) => (
              <div key={k} className="flex flex-wrap items-center gap-2">
                <input
                  value={e.nombre}
                  onChange={(ev) => guardarEscenarios(escenarios.map((x, i) => (i === k ? { ...x, nombre: ev.target.value } : x)))}
                ...
                <label className="flex items-center gap-1 text-xs text-zinc-400">
                  ...
                  <input
                    type="number"
                    value={e.ing}
                    onChange={(ev) => guardarEscenarios(escenarios.map((x, i) => (i === k ? { ...x, ing: Number(ev.target.value) } : x)))}
                ...
                <label className="flex items-center gap-1 text-xs text-zinc-400">
                  ...
                  <input
                    type="number"
                    value={e.gas}
                    onChange={(ev) => guardarEscenarios(escenarios.map((x, i) => (i === k ? { ...x, gas: Number(ev.target.value) } : x)))}
                    className="w-16 rounded-lg border border-zinc-800 bg-zinc-950 px-2 py-1 text-right text-xs text-white outline-none focus:border-red-500"
                  />
                  %
                </label>
                {escenarios.length > 1 && (
                  <button
                    onClick={() => guardarEscenarios(escenarios.filter((_, i) => i !== k))}
            ...
            {escenarios.length < 4 && (
              <button
                onClick={() => guardarEscenarios([...escenarios, { nombre: `Escenario ${escenarios.length + 1}`, ing: 0, gas: 0 }])}
            ...
            <p className="text-[10px] text-zinc-600">
              Cada escenario ajusta en % los ingresos y gastos recurrentes. Ej: Pesimista = ingresos −15% y gastos +10%.
            </p>
          </div>
        )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-800 text-left text-xs uppercase text-zinc-500">
                ...
                {escenarios.map((e, k) => (
                  <th key={k} className="py-2 text-right">{e.nombre}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {proyeccion.map((f) => (
                <tr key={f.mes} className="border-b border-zinc-800/60 last:border-0">
                  ...
                  {f.saldos.map((s, k) => (
                    <td key={k} className={`py-2.5 text-right font-bold ${s < 0 ? "text-red-400" : "text-white"}`}>
                      {eur(s)}
                    </td>
                  ))}
                </tr>
        ...
        {proyeccion.some((f) => f.saldos.some((s) => s < 0)) && (
          <p className="mt-3 rounded-lg bg-red-950 px-3 py-2 text-xs text-red-300">
            ⚠ Algún escenario se queda en negativo. Revisa gastos o adelanta cobros.
          </p>
        )}
      </div>
